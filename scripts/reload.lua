...
function reload_manual()
  if not mp.get_property_bool("idle-active") and
     not mp.get_property_bool("demuxer-via-network") then
    mp.commandv("loadfile", mp.get_property("path", property_path), "replace")
    msg.info("manual reload")
    mp.osd_message("Reset playback")
  end
end
...
if settings.reload_key_binding ~= "" then
  ...
  mp.add_key_binding(nil, "reload_resume", reload_resume)
end

mp.add_key_binding(nil, "manual", reload_manual)
